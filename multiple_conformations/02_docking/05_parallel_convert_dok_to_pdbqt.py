...
def _count_conformers(pdb_path, out_path):
...
def make_first_model_pdbqt(infile, temp_path):
    ...
    command = [MGL_PYTHON, PREPARE_SCRIPT, "-l", tmp_pdb, "-o", tmp_pdbqt, "-U", "\"''\""]
    try:
...
def main(datafile):
    ...
    for uniprot in database["UNIPROT"].values():
        for pdbid in uniprot["pdbs"]:
            conformers = _count_conformers("../01_fpocket/{}".format(pdbid), "receptors")
