...
def build_subdatabases(database, clusters, conformers):
...
def create_links(database):
    ...
    for uniprot in database["UNIPROT"].values():
        ...
        dock_folder = os.path.join("..", "02_docking", uniprot["pdbs"][0], "outputs")
        ...
        log_folder = os.path.join("..", "02_docking", uniprot["pdbs"][0], "logs")
        ...
        # LeDock results
        dock_folder = os.path.join("..", "02_docking", uniprot["pdbs"][0], "ledock")
        dockresults = [f for f in os.listdir(dock_folder) if f.endswith("pdbqt")]
        for dr in dockresults:
            os.symlink(os.path.join("..", "..", dock_folder, dr), os.path.join("docking", "outputs", dr))
    rec_folder = os.path.join("..", "02_docking", "receptors")
    receptors = [f for f in os.listdir(rec_folder) if not f.endswith("_LD.pdb")]
    sub_folder = os.path.join("..", "02_docking", "substrates")
