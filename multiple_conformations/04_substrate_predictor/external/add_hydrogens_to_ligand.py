...
def add_hydrogens_batch(directory, mode):
    if mode == "substrates":
        ...
        conformer_dirs = os.listdir(directory)
        for _dir in conformer_dirs:
            pool = mp.Pool(processes=ENV["num_parallel_cpu"])
            processing = []
            
            for subdir_name in sorted(os.listdir(os.path.join(directory, _dir, "sdf-hydrogens"))):
                if subdir_name.startswith("subdir"):
                    dir_to_process = os.path.join(directory, _dir, "sdf-hydrogens", subdir_name)
                    processing.append(pool.apply_async(_add_hydrogens_batch, args=(dir_to_process, _dir)))
