...
def align_receptors(pdbfile, out_path):
    aligned_pdb = "{}/{}".format(out_path, pdbfile)
    if not os.path.isfile(aligned_pdb):
        pymol_script = _pymol_align_script(REF_PDB, pdbfile, aligned_pdb)
        ...
        os.system("sed -i 's/Oa/ O/g' receptors/{}".format(pdbfile))
        os.system("sed -i 's/Op/ O/g' receptors/{}".format(pdbfile))
...
def main(datafile):
    ...
    for uniprot in database["UNIPROT"].values():
        for pdb in uniprot["pdbs"]:
            ...
            align_receptors("{}.pdb".format(pdb), "receptors")
            for substrate in uniprot["substrates"]:
                prepare_docking_files(pdb, substrate, box, conformers=1)
    align_receptors("{}.pdb".format(database["QUERY_PROTEIN"]["PDB"]), "receptors")
    pdb = "receptors/{}_c0.pdb".format(database["QUERY_PROTEIN"]["PDB"])
    pdbqt = "receptors/{}_c0.pdbqt".format(database["QUERY_PROTEIN"]["PDB"])
    if not os.path.isfile(pdbqt):
        _prepare_protein_pdbqt(pdb, pdbqt)
