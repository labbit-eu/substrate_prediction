...
class PubchemSearchByNameCompoundFactory(object):
    ...
    def create(self, query):
        ...
        try:
            ...
            query_type = "name"
        for compound in pubchempy.get_compounds(query, query_type):
            record = PubchemCompound(compound.cid)
            record.smiles = compound.canonical_smiles
...
def all_smiles_same(molecules):
    all_smiles = {}
    for mol in molecules:
        if mol.smiles in all_smiles.keys():
            all_smiles[mol.smiles] += 1
        else:
            all_smiles[mol.smiles] = 1
    return len(all_smiles) == 1
