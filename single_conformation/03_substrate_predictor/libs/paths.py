...
def grouped_filters():
    path = os.path.join(_DATA_DIR, "grouped_filters")
    os.makedirs(path, exist_ok=True)
    return path
...
def candidates_conf_sdf(conformer):
    path = os.path.join(candidates(), "conformer_c{}".format(conformer), "sdf")
    os.makedirs(path, exist_ok=True)
    return path
...
def candidates_conf_pdbqt(conformer):
    path = os.path.join(candidates(), "conformer_c{}".format(conformer), "pdbqt")
    os.makedirs(path, exist_ok=True)
    return path
...
def candidates_conf_docking(conformer):
    path = os.path.join(candidates(), "conformer_c{}".format(conformer), "docking")
    os.makedirs(path, exist_ok=True)
    return path
...
def candidates_conf_info(conformer):
    return os.path.join(candidates(), "info_c{}.json".format(conformer))
...
def target_conf_protein(conformer):
    return os.path.join(candidates(), "protein_c{}.pdbqt".format(conformer))
